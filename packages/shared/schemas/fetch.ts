...
export function validateFetchInput(input: unknown): FetchInput {
  ...
  return input as unknown as FetchInput;
}
...
export function validateFetchOutput(output: unknown): FetchOutput {
  ...
  if (!Array.isArray(output.metadata.section_path)) {
    throw new Error('metadata.section_path must be an array of strings');
  }
  output.metadata.section_path.forEach((h, i) => assertString(h, `metadata.section_path[${i}]`));
